...
W = tf.Variable(np.random.normal(
    0, 0.05, (256 + hiddenUnits + hiddenUnits, hiddenUnits)), dtype=tf.float32)
...
for timeTick in sequenceOfLetters:
    ...
    cachedStates[tick % 10] = next_state
...
with tf.Session(config=tf.ConfigProto(gpu_options=gpu_options)) as sess:
    ...
    saver.save(sess, 'checkPoint2.tf')
...
sess = tf.Session()
saver = tf.train.Saver()
saver.restore(sess, 'checkPoint2.tf')
