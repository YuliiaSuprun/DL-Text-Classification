...
W = tf.Variable(np.random.normal(
    0, 0.05, (8 + hiddenUnits + hiddenUnits, hiddenUnits)), dtype=tf.float32)
...
# Add 8 confolutional filters.
filters = tf.Variable(np.random.normal(
    0, 0.05, (10, 256, 8)), dtype=tf.float32)
...
cachedStates = [initialState] * 10
currentState = initialState
for i in range(maxSeqLen - 9):
    seq10 = tf.stack(sequenceOfLetters[i: i+10], axis=1)
    timeTick = tf.tensordot(seq10, filters, axes=((1, 2), (0, 1)))
    inputCurState = tf.concat([timeTick, currentState], 1)
    # get a state from 10 ticks ago.
    pastState = cachedStates[i % 10]
    # concatenate the input, current state, and past state.
    inputCurPastStates = tf.concat([inputCurState, pastState], 1)
    next_state = tf.tanh(tf.matmul(inputCurPastStates, W) + b)
    cachedStates[i % 10] = next_state
...
with tf.Session(config=tf.ConfigProto(gpu_options=gpu_options)) as sess:
    ...
    saver.save(sess, 'checkPoint5.tf')
...
sess = tf.Session()
saver = tf.train.Saver()
saver.restore(sess, 'checkPoint5.tf')
...
for k in range((int)(len(testData) / batchSize)):
    ...
    for i in range(len(yTest)):
        ...
        if maxPos == yTest[i]:
            totalCorrect += 1
print("Loss for", len(testData), "randomly chosen documents is", loss / (len(testData) / batchSize), ", number correct labels is", totalCorrect, "out of", len(testData))
